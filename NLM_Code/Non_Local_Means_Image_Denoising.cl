...
/////////////////Kernel for greyscale image denoisng/////////////////////////////
__kernel void NonLocalMeansFilter_Greyscale(__global float* input_image, __global float* input_Temp1,__global float* input_Temp2)
{ 
	...
	for (int local_y = 0; local_y < imgW - smallwindow + 1; local_y++)
	{
		float dist = 0;
		float Finaldist = 0;
		for (int m = local_x; m < local_x + smallwindow; m++)
		{
			for (int n = local_y; n < local_y + smallwindow; n++)
			{
				...
				dist = (input_image[(xdir + m - local_x) * imgW + ydir + n - local_y] - input_image[m * imgW + n]);
				Finaldist += (dist * dist);
			}
		}
		...
		input_Temp1[xdir * imgW + ydir] += weight * input_image[local_x * imgW + local_y];
		input_Temp2[xdir * imgW + ydir] += weight;
		input_Temp1[local_x * imgW + local_y] += weight * input_image[xdir * imgW + ydir];
		input_Temp2[local_x * imgW + local_y] += weight;
	}
} 
